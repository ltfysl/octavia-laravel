...
<template>
    <div class="min-h-screen lg:grid lg:grid-cols-[18rem_1fr] xl:grid-cols-[23.75rem_1fr]">
        ...
        <aside class="hidden lg:flex lg:h-screen lg:flex-col lg:border-r lg:border-ink-100 lg:bg-card/80 lg:backdrop-blur supports-[backdrop-filter]:bg-card/60">
            ...
            <Link href="/settings/workspace" class="mt-1 flex items-center gap-3 rounded-lg px-3 py-2 text-sm font-medium transition-colors" :class="isActive('/settings/workspace') ? 'bg-accent-50 text-accent-700' : 'text-ink-500 hover:bg-paper-100 hover:text-ink-900'">
                {{ t('settings.workspace') }}
            </Link>
            <Link href="/settings/provider-keys" class="mt-1 flex items-center gap-3 rounded-lg px-3 py-2 text-sm font-medium transition-colors" :class="isActive('/settings/provider-keys') ? 'bg-accent-50 text-accent-700' : 'text-ink-500 hover:bg-paper-100 hover:text-ink-900'">
        ...
        <div class="sticky top-0 z-30 flex items-center justify-between border-b border-ink-100 bg-card/90 px-4 py-3 backdrop-blur lg:hidden">
            ...
            <details class="relative">
                ...
                <nav class="absolute right-0 z-40 mt-2 w-56 rounded-card border border-ink-100 bg-card p-2 shadow-pop" aria-label="Mobile">
                    ...
                    <Link href="/settings/workspace" class="block rounded-lg px-3 py-2 text-sm text-ink-700">{{ t('settings.workspace') }}</Link>
                    <Link href="/settings/provider-keys" class="block rounded-lg px-3 py-2 text-sm text-ink-700">{{ t('settings.providerKeys.title') }}</Link>
