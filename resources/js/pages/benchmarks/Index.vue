...
<template>
    <AppLayout>
        ...
        <div class="flex items-start justify-between gap-4">
            ...
            <Link href="/benchmarks/wizard" class="rounded-lg bg-violet-600 px-4 py-2 text-sm font-medium text-white transition-colors hover:bg-violet-700">
                + {{ t('benchmarks.new') }}
            </Link>
        </div>
        ...
        <OEmptyState v-if="benchmarks.length === 0" class="mt-8" :title="t('benchmarks.empty')">
            <template #action>
                <Link href="/benchmarks/wizard" class="rounded-lg bg-violet-600 px-4 py-2 text-sm font-medium text-white hover:bg-violet-700">{{ t('benchmarks.wizard.title') }}</Link>
            </template>
        </OEmptyState>

        <ul v-else class="mt-8 grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
            <li v-for="benchmark in benchmarks" :key="benchmark.id">
                <Link :href="`/benchmarks/${benchmark.id}`" class="group flex h-full flex-col rounded-card border border-ink-100 bg-white p-5 shadow-panel transition-colors hover:border-violet-200">
                    <div class="flex items-center justify-between gap-2">
                        <h2 class="truncate font-display text-sm font-semibold text-ink-950 group-hover:text-violet-700">{{ benchmark.name }}</h2>
