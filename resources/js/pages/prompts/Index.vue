...
const duplicatePrompt = (id: number) => router.post(`/prompts/${id}/duplicate`);
</script>
    ...
    <AppLayout>
        ...
        <div class="flex flex-wrap items-start justify-between gap-4">
            <div>
                <div class="flex items-center gap-2">
                    ...
                    <p class="eyebrow">{{ t('prompts.specimenLibrary') }}</p>
        ...
        <div class="mt-6 flex flex-wrap items-center gap-3">
            <div class="flex flex-1 min-w-[220px] items-center gap-2 rounded-full border border-ink-200 bg-card px-3 py-2 shadow-sm">
                ...
                <input v-model="searchQuery" placeholder="{{ t('prompts.searchPlaceholder') }}" class="flex-1 bg-transparent text-sm outline-none placeholder:text-ink-400" />
            ...
            <div class="flex items-center gap-1 rounded-full bg-ink-100 p-1">
                <button type="button" class="rounded-full px-3 py-1.5 text-xs font-medium transition" :class="visibilityFilter === 'all' ? 'bg-card text-ink-900 shadow-sm' : 'text-ink-500 hover:text-ink-700'" @click="visibilityFilter = 'all'">{{ t('common.all') }}</button>
                <button type="button" class="rounded-full px-3 py-1.5 text-xs font-medium transition" :class="visibilityFilter === 'private' ? 'bg-card text-ink-900 shadow-sm' : 'text-ink-500 hover:text-ink-700'" @click="visibilityFilter = 'private'">{{ t('common.private') }}</button>
                <button type="button" class="rounded-full px-3 py-1.5 text-xs font-medium transition" :class="visibilityFilter === 'public' ? 'bg-card text-ink-900 shadow-sm' : 'text-ink-500 hover:text-ink-700'" @click="visibilityFilter = 'public'">{{ t('common.public') }}</button>
            </div>
        ...
        <div v-else-if="filtered.length === 0" class="mt-8 rounded-card border border-dashed border-ink-200 bg-card p-8 text-center">
            <p class="text-sm font-medium text-ink-900">{{ t('prompts.noSearchMatches') }} “{{ searchQuery }}”</p>
            <p class="mt-1 text-sm text-ink-500">{{ t('prompts.noSearchHint') }}</p>
            <button type="button" class="mt-4 rounded-full bg-ink-950 px-4 py-1.5 text-sm font-medium text-white hover:bg-ink-900" @click="searchQuery=''; visibilityFilter='all'">{{ t('common.clearFilters') }}</button>
        </div>
                    ...
                    <div class="flex h-full flex-col p-5">
                        ...
                        <div class="mt-4 flex items-center justify-between border-t border-dashed border-ink-100 pt-3">
                            ...
                            <div class="flex items-center gap-2">
                                ...
                                <button type="button" :title="t('common.duplicate')" class="rounded-md p-1 text-ink-400 transition hover:bg-paper-100 hover:text-ink-700" @click.stop.prevent="duplicatePrompt(prompt.id)">
